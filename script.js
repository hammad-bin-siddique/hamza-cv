  ...
  (() => {
    ...
    document.getElementById('themeIco').className   = saved==='dark' ? 'fas fa-sun'  : 'fas fa-moon';
    document.getElementById('themeLbl').textContent = saved==='dark' ? 'Light Mode' : 'Dark Mode';
  })();

  /* Scroll reveal */
  const revItems = document.querySelectorAll('.cv-card,.lang-c,.hob-tag,.obj-box');
  const ro = new IntersectionObserver((entries)=>{
    entries.forEach((e,i)=>{
      if(e.isIntersecting){
        e.target.style.opacity='0'; e.target.style.transform='translateY(22px)';
        setTimeout(()=>{
  ...
  revItems.forEach(el=>{ el.style.opacity='0'; ro.observe(el); });

  /* Skill bars */
  const fills = document.querySelectorAll('.sk-fill');
  const so = new IntersectionObserver((entries)=>{
    entries.forEach(e=>{
      if(e.isIntersecting){ e.target.style.animation='barFill 1.5s cubic-bezier(.4,0,.2,1) both'; so.unobserve(e.target); }
    });
  },{threshold:.3});
